...
namespace lpt {
	...
	TrajectoryPathVTK::TrajectoryPathVTK(vtkRenderer* renderer): renderer(renderer) {	
...
//Pressure Calculation
	...
	inline void ParticlesVTK::setVectorMode(lpt::VectorMode mode) { 
		vector_mode = mode; 
		switch (mode) {
		case lpt::VELOCITY:
				scalarbar->GetScalarBarActor()->SetTitle("Velocity Magnitude (m/s)");
	...
	Visualizer::Visualizer() : visualization_status(false), accumulate_status(false), take_measurement(false), image_measurement(false), measurement_count(0) {
	...
	void Visualizer::manageQueue() {
		...
		while(true) {
			...
			for ( auto traj_iter = traj_updates.begin(); traj_iter != traj_updates.end(); ++traj_iter, ++id) {
				...
				auto& X0 = iter_previous->get()->X;
				auto& X1 = object_iter->get()->X;
				auto& X2 = iter_next->get()->X;
				...
				for (int d = 0; d < X0.size(); ++d) {
					new_vectors[0][d] = X1[d];
					new_vectors[1][d] = ( ( X1[d] - X0[d] ) * frame_rate + ( X2[d] - X1[d] ) * frame_rate ) / 2.0 / 1000.0;
					new_vectors[2][d] = ( X2[d] - 2.0 * X1[d] + X0[d] ) * frame_rate * frame_rate / 1000.0;
					new_vectors[3][d] = 0;
				}

				if (iter_next != ( current_traj->objects.end() - 1 ) && iter_previous != current_traj->objects.begin() ) {
					auto& Xm1 = (iter_previous - 1)->get()->X;
					auto& X3 = (iter_next + 1)->get()->X;
